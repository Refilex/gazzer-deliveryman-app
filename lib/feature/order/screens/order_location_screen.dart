...
import 'package:gazzer_delivery/feature/order/widgets/location_card_widget.dart';
...
class _OrderLocationScreenState extends State<OrderLocationScreen> {
  ...
  void _setMarker(OrderModel orderModel) async {
    try {
      ...
      LatLngBounds? bounds;
      if (_controller != null) {
        if ((Get.find<ProfileController>().recordLocationBody?.latitude ?? 0) <
            double.parse(orderModel.restaurantLat ?? '0')) {
          bounds = LatLngBounds(
            southwest: LatLng(
                Get.find<ProfileController>().recordLocationBody?.latitude ?? 0,
                Get.find<ProfileController>().recordLocationBody?.latitude ??
                    0),
            northeast: LatLng(double.parse(orderModel.restaurantLat ?? '0'),
                double.parse(orderModel.restaurantLng ?? '0')),
          );
        } else {
          bounds = LatLngBounds(
            southwest: LatLng(double.parse(orderModel.restaurantLat ?? '0'),
                double.parse(orderModel.restaurantLng ?? '0')),
            northeast: LatLng(
                Get.find<ProfileController>().recordLocationBody?.latitude ?? 0,
      ...
      orderModel.restaurantLat != null
          ? _markers.add(Marker(
              markerId: const MarkerId('restaurant'),
              position: LatLng(double.parse(orderModel.restaurantLat ?? '0'),
                  double.parse(orderModel.restaurantLng ?? '0')),
              infoWindow: InfoWindow(
      ...
      Get.find<ProfileController>().recordLocationBody != null
          ? _markers.add(Marker(
